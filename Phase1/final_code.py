...
def main():
    ...
    metrics = evaluate_silence_from_masks(
    W=W,
    gt_mask=X_act,
    pred_mask=mask_gnn,
    coords=src_xyz)
    print("Mean Jaccard:", metrics["mean_jaccard"])
    print("Mean COM error:", metrics["mean_com_error"])
    print("Mean size error:", metrics["mean_size_error"])
    ...
    for th in [0.9]:
       mask_gnn = (prob_np >= th)
       P, R, F1 = pr_re_f1(mask_gnn, X_act)
       print(f"th={th:.2f} pred={int(mask_gnn.sum())}  P={P:.3f} R={R:.3f} F1={F1:.3f}")
