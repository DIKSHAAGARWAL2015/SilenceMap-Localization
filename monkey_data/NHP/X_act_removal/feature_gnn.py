...
def main():
    ...
    parser.add_argument("--K", type=int, default=4, help="Number of silent regions")
    parser.add_argument("--per_region_k", type=int, default=12, help="Nodes per region")
    ...
    t, Fs = args.t, args.Fs
    # if src_xyz is in meters (typical), convert to mm for this masking logic
    #src_xyz_mm = src_xyz * 1000.0
    #allowed = np.where((src_xyz_mm[:,0] <= -mid_gap_mm) | (src_xyz_mm[:,0] >= mid_gap_mm))[0]

    # disallow midline band (optional)
    mid_gap_mm = 5.0
    #src_xyz = src_xyz * 1000
    ...
    X_act = np.zeros(src_xyz.shape[0], dtype=bool)
    ...
    target_snr_db = 10  # e.g. 5, 10, 15
    sig_pow = np.mean(eeg_lp**2)
    noise_pow = sig_pow / (10**(target_snr_db/10))

    E = rng.standard_normal(eeg_lp.shape) * np.sqrt(noise_pow)
    eeg = eeg_lp + E
    snr = 10*np.log10(np.mean(eeg_lp**2) / (np.mean(E**2)+1e-12))
    print(f"Avg SNR ≈ {snr:.2f} dB (target={target_snr_db} dB)")
    # SNR sanity
    ...
    for th in [0.5, 0.7, 0.8, 0.9, 0.95, 0.98]:
         mask_gnn = (prob_np >= th)
         P,R,F1 = pr_re_f1(mask_gnn, X_act)
         print(f"th={th:.2f} pred={m.sum()}  P={P:.3f} R={R:.3f} F1={F1:.3f}")
    #mask_gnn = grow_k_regions(W, prob, K=K, k_total=k_silent)

    m = component_metrics(W, X_act, mask_gnn)
    print_component_report("GNN", m)
    """
    # prob should be a 1D numpy array
    prob_np = prob.reshape(-1)  # if already 1D, this is harmless

    best = (-1.0, None, None)  # (F1, th, mask)

    print("GT positives:", int(X_act.sum()))
    for th in [0.5, 0.7, 0.8, 0.9, 0.95, 0.98]:
       mask = (prob_np >= th)
       P, R, F1 = pr_re_f1(mask, X_act)
       print(f"th={th:.2f} pred={int(mask.sum())}  P={P:.3f} R={R:.3f} F1={F1:.3f}")

       if F1 > best[0]:
          best = (F1, th, mask)

# report components for the best threshold (DEBUG only; don't choose on test later)
    best_f1, best_th, best_mask = best
    print(f"[debug] best_th={best_th:.2f} best_F1={best_f1:.3f}")

    m = component_metrics(W, X_act, best_mask)
    print_component_report("GNN(best_th)", m)
