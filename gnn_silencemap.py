...
def main():
    ...
    parser.add_argument("--leadfield_path", type=str, default="/content/OT_leadfield_symmetric_1662-128.mat")
    parser.add_argument("--headmodel_path", type=str, default="/content/OT_headmodel_symmetric_1662-128.mat")
    ...
    # -------- Load cortex/sensors/leadfield --------
    if args.use_mat:
        print("Loading .mat leadfield/headmodel...")
        try:
            L = loadmat(args.leadfield_path)["L"].astype(np.float32)  # (n,p)
            hm = loadmat(args.headmodel_path)
            Cortex = hm.get("Cortex", None)
            if Cortex is None:
                # sometimes stored differently; try direct field
                src_xyz = hm["Cortex.Pial.vertices"].astype(np.float32)
            else:
                Cortex = Cortex[0,0]
                src_xyz = Cortex["Pial"][0,0]["vertices"].astype(np.float32)
            n, p = L.shape
            print("Loaded L:", L.shape, "sources:", src_xyz.shape)
